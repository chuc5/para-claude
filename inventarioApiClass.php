...
final class inventarioApiClass extends ConexionBD
{
    ...
    // ============================================================================
//? TIPOS DE PRODUCTO — Funciones para agregar a la clase existente
// ============================================================================
// Reglas de negocio:
//  - Catálogo cerrado: solo 3 tipos definidos en la semilla de BD
//  - Todos los usuarios pueden LEER
//  - Solo admin puede EDITAR nombre y descripción
//  - PROHIBIDO crear y eliminar (cambiar el tipo requiere cambios de código)
// ============================================================================
...
// ── GET: listarTipos ──────────────────────────────────────────────────────────
    ...
    /**
     * Devuelve los 3 tipos del catálogo semilla.
     * Acceso: todos los usuarios autenticados.
     *
     * GET bodega/catalogo/listarTipos
     */
    public function listarTipos(): array
    {
        try {
            ...
            return $this->res->ok('Tipos de producto obtenidos', [
                'tipos' => $tipos, // Ahora 'tipos' contiene una lista de objetos stdClass
                'total' => count($tipos),
            ], ['es_admin' => $this->_esTiposAdmin()]);

        } catch (Exception $e) {
            error_log("[TiposProducto] listarTipos: " . $e->getMessage());
            return $this->res->fail('Error al obtener los tipos de producto', $e);
        }
    }


// ── GET: obtenerTipo?id=X ─────────────────────────────────────────────────────

    /**
     * Obtiene un único tipo por su ID.
     * Acceso: todos los usuarios autenticados.
     *
     * GET bodega/catalogo/obtenerTipo?id=1
     */
    public function obtenerTipo(): array
    {
        try {
         ...
         WHERE id = ?"
            ...
            // Como es una fila única, fetch() nos devolverá un solo objeto stdClass (o false si no existe)
            $tipo = $stmt->fetch(PDO::FETCH_OBJ);

            if (!$tipo) {
                return $this->res->fail("El tipo de producto con id={$id} no existe en el catálogo");
            }

            // Ahora $tipo es un objeto, y se envía directamente en la respuesta
            return $this->res->ok('Tipo de producto obtenido', $tipo);
        } catch (Exception $e) {
            error_log("[TiposProducto] obtenerTipo: " . $e->getMessage());
...
// ── POST: editarTipo ──────────────────────────────────────────────────────────
    ...
    /**
     * Edita nombre y/o descripción de un tipo (solo admin).
     * El campo id NO puede modificarse — es parte de la semilla.
     *
     * POST bodega/catalogo/editarTipo
     * Body: { "id": 1, "nombre": "Correlativo", "descripcion": "..." }
     *
     * @param object|array $datos Cuerpo de la petición deserializado
     */
    public function editarTipo($datos): array
    {
        try {
            ...
            $datos = $this->limpiarDatos($datos);   // limpiarDatos ya existe en la clase
            ...
            // ── Verificar que el tipo existe en la semilla ────────────────────────
            $stmtExiste = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.tipos_producto WHERE id = ?"
            );
            $stmtExiste->execute([$id]);

            // Nota: fetchColumn() funciona igual independientemente de FETCH_OBJ/FETCH_ASSOC
            if (!$stmtExiste->fetchColumn()) {
                return $this->res->fail("El tipo con id={$id} no existe en el catálogo");
            }

            // ── Verificar unicidad del nombre (excluye el propio registro) ─────────
            $stmtDup = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.tipos_producto
         WHERE nombre = ? AND id != ?"
            );
            $stmtDup->execute([$nombre, $id]);

            if ($stmtDup->fetchColumn()) {
                return $this->res->fail("Ya existe otro tipo de producto con el nombre \"{$nombre}\"");
            }

            // ── Actualizar (solo nombre y descripción) ────────────────────────────
            $stmtUpdate = $this->connect->prepare(
                "UPDATE bodega_inventario.tipos_producto
         SET nombre = ?, descripcion = ?
         WHERE id = ?"
            );
            $stmtUpdate->execute([$nombre, $descripcion, $id]);

            return $this->res->ok('Tipo de producto actualizado correctamente');
        } catch (Exception $e) {
            error_log("[TiposProducto] editarTipo: " . $e->getMessage());
            return $this->res->fail('Error al actualizar el tipo de producto', $e);
        }
    }
...
// ── Helper privado ────────────────────────────────────────────────────────────
     ...
     * Prefijo _esTiposAdmin para no colisionar si ya existe un método esAdmin() en la clase.
     * Ajusta $puestosAdmin según tu configuración.
     */
    private function _esTiposAdmin(): bool
    {
        $puestosAdmin = [1, 3, 56];
        return in_array($this->puesto, $puestosAdmin, true);
    }
...
// ── POST: editarUnidad ────────────────────────────────────────────────────────
    ...
    /**
     * Edita nombre y/o abreviatura de una unidad existente.
     *
     * POST bodega/catalogo/editarUnidad
     * Body: { "id": 1, "nombre": "Libra", "abreviatura": "Lb" }
     *
     * @param object|array $datos
     */
    public function editarUnidad($datos): array
    {
        try {
            ...
            // ── Verificar existencia ──────────────────────────────────────────────
            $stmtExiste = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.unidades_medida WHERE id = ?"
            );
            $stmtExiste->execute([$id]);
            if (!$stmtExiste->fetchColumn()) {
                return $this->res->fail("La unidad con id={$id} no existe");
            }

            // ── Unicidad de abreviatura (excluye el propio registro) ───────────────
            $stmtDupAbrev = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.unidades_medida
             WHERE abreviatura = ? AND id != ?"
            );
            $stmtDupAbrev->execute([$abreviatura, $id]);
            if ($stmtDupAbrev->fetchColumn()) {
                return $this->res->fail("Ya existe otra unidad con la abreviatura \"{$abreviatura}\"");
            }

            // ── Unicidad de nombre (excluye el propio registro) ───────────────────
            $stmtDupNombre = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.unidades_medida
             WHERE nombre = ? AND id != ?"
            );
            $stmtDupNombre->execute([$nombre, $id]);
            if ($stmtDupNombre->fetchColumn()) {
                return $this->res->fail("Ya existe otra unidad con el nombre \"{$nombre}\"");
            }

            // ── Actualizar ────────────────────────────────────────────────────────
            $stmt = $this->connect->prepare(
                "UPDATE bodega_inventario.unidades_medida
            ...
            error_log("[Unidades] editarUnidad: " . $e->getMessage());
...
// ── POST: toggleUnidad ────────────────────────────────────────────────────────
     ...
     *
     * BLOQUEO al desactivar (fail — la operación NO se completa):
     *   1. La unidad es la unidad default de algún producto activo
     *   2. La unidad tiene stock > 0 en alguna bodega
     *      ↳ Cuando las tablas de stock/lotes estén definidas, actualizar
     *        la consulta marcada con TODO al final de esta función.
     *
     * POST bodega/catalogo/toggleUnidad
     * Body: { "id": 1, "activo": 0 }   ← 0 desactiva, 1 activa
     *
     * @param object|array $datos
     */
    public function toggleUnidad($datos): array
    {
        try {
         ...
         WHERE id = ?"
            ...
            // CAMBIO 2: Acceso al estado actual como propiedad del objeto
            if ((int)$unidad->activo === $activo) {
                $estado = $activo === 1 ? 'activa' : 'inactiva';
                return $this->res->info("La unidad ya se encuentra {$estado}");
            }

            // ── Validaciones de bloqueo al desactivar ─────────────────────────────
            if ($activo === 0) {

                // Bloqueo 1: unidad default de algún producto activo
                $stmtDefault = $this->connect->prepare(
                    "SELECT COUNT(*)
             ...
             WHERE pu.id_unidad = ?
               ...
               AND p.activo      = 1"
                ...
                $stmtDefault->execute([$id]);
                $totalDefault = (int)$stmtDefault->fetchColumn();

                if ($totalDefault > 0) {
                    // CAMBIO 3: Acceso al nombre como propiedad ($unidad->nombre)
                    return $this->res->fail(
                        "No se puede desactivar: la unidad \"{$unidad->nombre}\" es la unidad " .
                        "principal de {$totalDefault} producto(s) activo(s). " .
                        "Cambia primero la unidad default de esos productos."
                    );
                }

                // Bloqueo 2: stock o asociación activa
                $stmtEnUso = $this->connect->prepare(
                    "SELECT COUNT(*)
             FROM bodega_inventario.productos_unidades pu
             INNER JOIN bodega_inventario.productos p ON p.id = pu.id_producto
             WHERE pu.id_unidad = ?
               ...
               AND p.activo     = 1"
                ...
                $stmtEnUso->execute([$id]);
                $totalEnUso = (int)$stmtEnUso->fetchColumn();

                if ($totalEnUso > 0) {
                    // CAMBIO 4: Acceso al nombre como propiedad ($unidad->nombre)
                    return $this->res->fail(
                        "No se puede desactivar: la unidad \"{$unidad->nombre}\" tiene " .
                        "stock activo en {$totalEnUso} producto(s). " .
                        "Retira primero el stock antes de desactivarla."
                    );
                }
            }
            ...
            // ── Actualizar estado ─────────────────────────────────────────────────
            $stmt = $this->connect->prepare(
                "UPDATE bodega_inventario.unidades_medida SET activo = ? WHERE id = ?"
            );
            $stmt->execute([$activo, $id]);
            ...
            error_log("[Unidades] toggleUnidad: " . $e->getMessage());
...
// ── POST: editarProducto ──────────────────────────────────────────────────────
    ...
    /**
     * Edita nombre, descripción, categoría y unidades.
     * El tipo (id_tipo) NO puede modificarse.
     * Gestión de unidades dentro de la misma transacción.
     *
     * POST bodega/catalogo/editarProducto
     * Body: { "id": 1, "nombre": "...", "id_categoria": 2, "unidades": [...] }
     *
     * @param object|array $datos
     */
    public function editarProducto($datos): array
    {
        try {
            ...
            $stmtExiste = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.productos WHERE id = ?"
            );
            $stmtExiste->execute([$id]);
            if (!$stmtExiste->fetchColumn()) {
                return $this->res->fail("El producto con id={$id} no existe");
            ...
            // Unicidad del nombre (excluye el propio)
            $stmtDup = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.productos WHERE nombre = ? AND id != ?"
            );
            $stmtDup->execute([trim($datos->nombre), $id]);
            if ($stmtDup->fetchColumn()) {
                return $this->res->fail("Ya existe otro producto con el nombre \"{$datos->nombre}\"");
            }
            ...
            // Obtener unidades actuales activas
            $stmtActuales = $this->connect->prepare(
                "SELECT id_unidad FROM bodega_inventario.productos_unidades 
            ...
            // Validar que no se quiten unidades con stock activo
            ...
            $descripcion = !empty($datos->descripcion) && trim($datos->descripcion) !== ''
                ? trim($datos->descripcion)
                : null;
            ...
            $stmtProd = $this->connect->prepare(
                "UPDATE bodega_inventario.productos 
             SET nombre = ?, descripcion = ?, id_categoria = ? 
             WHERE id = ?"
            );
            $stmtProd->execute([trim($datos->nombre), $descripcion, (int)$datos->id_categoria, $id]);

            // Desactivar unidades removidas
            if (!empty($aRemover)) {
                $placeholders = implode(',', array_fill(0, count($aRemover), '?'));
                $stmtRemover = $this->connect->prepare(
                    "UPDATE bodega_inventario.productos_unidades 
                 SET activo = 0 
                 WHERE id_producto = ? AND id_unidad IN ($placeholders)"
                );
                $stmtRemover->execute(array_merge([$id], array_values($aRemover)));
            }

            // Insertar / Reactivar unidades
            foreach ($unidades as $u) {
                $idUnidad = (int)($u['id_unidad'] ?? ($u->id_unidad ?? 0));
                $esDefault = (int)($u['es_default'] ?? ($u->es_default ?? 0));

                if ($idUnidad < 1) continue;

                // Ver si ya existe (activa o inactiva)
                $stmtCheck = $this->connect->prepare(
                    "SELECT id FROM bodega_inventario.productos_unidades 
                ...
                if ($existe) {
                    ...
                    $this->connect->prepare(
                        "UPDATE bodega_inventario.productos_unidades 
                     SET activo = 1, es_default = ? 
                     WHERE id_producto = ? AND id_unidad = ?"
                    )->execute([$esDefault, $id, $idUnidad]);
                } else {
                    // Nueva unidad
                    $this->connect->prepare(
                        "INSERT INTO bodega_inventario.productos_unidades 
                     (id_producto, id_unidad, es_default) 
                     VALUES (?, ?, ?)"
                    )->execute([$id, $idUnidad, $esDefault]);
                }
            }

            // Asegurar que solo exista una unidad por defecto
    ...
    private function _normalizarDefaultUnidad(int $idProducto): void
    {
        ...
        // Poner default a la primera unidad activa
        $this->connect->prepare(
            "UPDATE bodega_inventario.productos_unidades 
         SET es_default = 1 
         WHERE id_producto = ? AND activo = 1 
...
// ── POST: toggleProducto ──────────────────────────────────────────────────────
     ...
     * Al desactivar el producto desaparece del catálogo pero conserva el stock.
     *
     * POST bodega/catalogo/toggleProducto
     * Body: { "id": 1, "activo": 0 }
     *
     * @param object|array $datos
     */
    public function toggleProducto($datos): array
    {
        try {
            ...
            if (!in_array($activo, [0, 1], true)) {
                return $this->res->fail('El campo activo debe ser 0 o 1');
            }

            $stmtExiste = $this->connect->prepare(
                "SELECT id, nombre, activo FROM bodega_inventario.productos WHERE id = ?"
            );
            $stmtExiste->execute([$id]);
            ...
            // CAMBIO 2: Acceso al estado actual como propiedad del objeto ($producto->activo)
            if ((int)$producto->activo === $activo) {
                $estado = $activo === 1 ? 'activo' : 'inactivo';
                return $this->res->info("El producto ya se encuentra {$estado}");
            }

            $this->connect->prepare(
                "UPDATE bodega_inventario.productos SET activo = ? WHERE id = ?"
            )->execute([$activo, $id]);
            ...
            // info() al desactivar para que el frontend muestre la nota sobre el stock
            return $activo === 0
                ? $this->res->info($mensaje)
                : $this->res->ok($mensaje);
        } catch (Exception $e) {
            error_log("[Productos] toggleProducto: " . $e->getMessage());
...
// ── Helpers privados ──────────────────────────────────────────────────────────
    ...
    /**
     * Valida los campos comunes de producto (crear y editar).
     * En modo editar omite la validación de id_tipo.
     *
     * @return array|null  null = válido; array = respuesta fail() con el error
     */
    ...
    /**
     * Normaliza el array de unidades recibido (puede ser array de objetos o arrays).
     */
    private function _normalizarUnidades($raw): array
    {
        if (!is_array($raw)) return [];
        return array_map(fn($u) => [
            'id_unidad' => (int)(is_object($u) ? $u->id_unidad : ($u['id_unidad'] ?? 0)),
            'es_default' => (bool)(is_object($u) ? $u->es_default : ($u['es_default'] ?? false)),
        ], $raw);
    }

    /**
     * Valida que el array de unidades cumpla con las reglas de negocio.
     *
     * @return array|null  null = válido; array = respuesta fail()
     */
    private function _validarUnidades(array $unidades): ?array
    {
        ...
        $defaults = array_filter($unidades, fn($u) => $u['es_default'] === true);
    ...
    /** Inserta las unidades de un producto nuevo en productos_unidades */
    private function _insertarUnidades(int $idProducto, array $unidades): void
    {
        $stmt = $this->connect->prepare(
            "INSERT INTO bodega_inventario.productos_unidades
         (id_producto, id_unidad, es_default) VALUES (?, ?, ?)"
        );
        foreach ($unidades as $u) {
            $stmt->execute([$idProducto, $u['id_unidad'], (int)$u['es_default']]);
        }
    }

    /** Verifica si un tipo de producto existe */
    ...
    {
        $stmt = $this->connect->prepare(
            "SELECT id FROM bodega_inventario.categorias_producto WHERE id = ? AND activo = 1"
        );
        $stmt->execute([$id]);
        return (bool)$stmt->fetchColumn();
    }
    ...
    /**
     * Verifica si una unidad de un producto tiene stock activo.
     * Retorna fail() si hay stock, null si puede removerse.
     *
     * TODO: reemplazar la consulta cuando las tablas de stock/lotes estén definidas.
     * Ejemplo futuro:
    ...
    private function _tieneStockActivo(int $idProducto, int $idUnidad): ?array
    {
     ...
     WHERE id_producto = ? AND id_unidad = ?"
        ...
        if ((float)$stmt->fetchColumn() > 0) {
            return $this->res->fail(
                "No se puede quitar: la unidad tiene stock activo en bodega"
            );
        }
        return null;
    }
...
// ── POST: editarBodega ────────────────────────────────────────────────────────
    ...
    /**
     * Edita nombre y vínculo (agencia/departamento) de una bodega.
     * El tipo NO puede modificarse.
     *
     * POST bodega/bodegas/editarBodega
     * Body tipo agencia: { "id": 1, "nombre": "...", "id_agencia": 5 }
     * Body tipo área:    { "id": 1, "nombre": "...", "id_departamento_cooperativa": 3 }
     *
     * @param object|array $datos
     */
    public function editarBodega($datos): array
    {
        try {
            $datos = $this->limpiarDatos($datos);
            $id = (int)($datos->id ?? 0);
            if ($id < 1) return $this->res->fail('El campo id es requerido');

            $stmtExiste = $this->connect->prepare(
                "SELECT id, id_tipo FROM bodega_inventario.bodegas WHERE id = ?"
            );
            $stmtExiste->execute([$id]);
            $bodega = $stmtExiste->fetch(PDO::FETCH_OBJ);
            if (!$bodega) return $this->res->fail("La bodega con id={$id} no existe");

            $idTipo = (int)$bodega->id_tipo;

            if ($idTipo === 1) {
                $idAgencia = (int)($datos->id_agencia ?? 0);
                if ($idAgencia < 1) return $this->res->fail('La agencia es requerida');
                ...
                if (!$nombre) return $this->res->fail("La agencia id={$idAgencia} no existe");
                if ($this->_agenciaTieneBodegaActiva($idAgencia, $id)) {
                    return $this->res->fail('Esta agencia ya tiene otra bodega activa');
                }
            } else {
                $idDepto = (int)($datos->id_departamento_cooperativa ?? 0);
                if ($idDepto < 1) return $this->res->fail('El departamento es requerido');
                ...
                if (!$nombre) return $this->res->fail("El departamento id={$idDepto} no existe");
                if ($this->_departamentoTieneBodegaActiva($idDepto, $id)) {
                    return $this->res->fail('Este departamento ya tiene otra bodega activa');
                }
            }
            ...
            error_log("[Bodegas] editarBodega: " . $e->getMessage());
...
// ── POST: toggleBodega ────────────────────────────────────────────────────────
     ...
     * Bloquea la desactivación si tiene stock > 0 o solicitudes/traslados pendientes.
     *
     * POST bodega/bodegas/toggleBodega
     * Body: { "id": 1, "activo": 0 }
     *
     * @param object|array $datos
     */
    public function toggleBodega($datos): array
    {
        try {
            ...
            $stmtExiste = $this->connect->prepare(
                "SELECT id, nombre, activo FROM bodega_inventario.bodegas WHERE id = ?"
            );
            $stmtExiste->execute([$id]);
            $bodega = $stmtExiste->fetch(PDO::FETCH_OBJ);
            ...
            if ((int)$bodega->activo === $activo) {
                $estado = $activo === 1 ? 'activa' : 'inactiva';
                return $this->res->info("La bodega ya se encuentra {$estado}");
            }

            // ── Bloqueos al desactivar ────────────────────────────────────────────
            if ($activo === 0) {
                $stmtStock = $this->connect->prepare(
                    "SELECT COALESCE(SUM(cantidad_total), 0)
                    FROM bodega_inventario.stock WHERE id_bodega = ?"
                );
                $stmtStock->execute([$id]);
                if ((float)$stmtStock->fetchColumn() > 0) {
                    return $this->res->fail(
                        "No se puede desactivar: la bodega tiene stock activo"
                    );
                }

                $stmtSol = $this->connect->prepare(
                    "SELECT COUNT(*) FROM bodega_inventario.solicitudes
                WHERE id_bodega = ? AND id_estado NOT IN (3, 4)"
                );
                $stmtSol->execute([$id]);
                if ((int)$stmtSol->fetchColumn() > 0) {
                    return $this->res->fail(
                        "No se puede desactivar: la bodega tiene solicitudes pendientes"
                    );
                }
            }

            $this->connect->prepare(
                "UPDATE bodega_inventario.bodegas SET activo = ? WHERE id = ?"
            )->execute([$activo, $id]);
            ...
            error_log("[Bodegas] toggleBodega: " . $e->getMessage());
            return $this->res->fail('Error al cambiar el estado de la bodega', $e);
        }
    }


// ── POST: toggleRestriccionAcceso ─────────────────────────────────────────────

    /**
     * Activa o desactiva la restricción de acceso de una bodega de área.
     * Solo aplica a bodegas con id_tipo = 2.
     * Si se desactiva, la matriz de acceso se conserva para reactivar después.
     *
     * POST bodega/bodegas/toggleRestriccionAcceso
     * Body: { "id": 1, "restriccion_acceso_activa": 1 }
     *
     * @param object|array $datos
     */
    public function toggleRestriccionAcceso($datos): array
    {
        try {
            ...
            $restriccion = isset($datos->restriccion_acceso_activa)
                ? (int)$datos->restriccion_acceso_activa : -1;
            ...
            $stmtExiste = $this->connect->prepare(
                "SELECT id, id_tipo, restriccion_acceso_activa
            ...
            // Solo bodegas de área
            if ((int)$bodega->id_tipo !== 2) {
                return $this->res->fail('La restricción de acceso solo aplica a bodegas de área');
            }

            if ((int)$bodega->restriccion_acceso_activa === $restriccion) {
                $estado = $restriccion === 1 ? 'activada' : 'desactivada';
                return $this->res->info("La restricción de acceso ya se encuentra {$estado}");
            }
            ...
            $this->connect->prepare(
             ...
             WHERE id = ?"
            )->execute([$restriccion, $id]);

            if ($restriccion === 0) {
                // La matriz se conserva — solo informar
                return $this->res->info(
                    'Restricción desactivada. La matriz de acceso se conserva para cuando se reactive.'
                );
            }

            return $this->res->ok('Restricción de acceso activada correctamente');
        } catch (Exception $e) {
            error_log("[Bodegas] toggleRestriccionAcceso: " . $e->getMessage());
...
// ── Helpers privados ──────────────────────────────────────────────────────────
    ...
    /**
     * Valida los campos comunes de bodega.
     *
     * @return array|null  null = válido; array = fail()
     */
    private function _validarCamposBodega(object $datos, bool $esEdicion): ?array
    {
        $nombre = trim($datos->nombre ?? '');
        if (empty($nombre)) {
            return $this->res->fail('El nombre de la bodega es requerido');
        }
        if (mb_strlen($nombre) > 100) {
            return $this->res->fail('El nombre no puede superar 100 caracteres');
        }
    ...
    /** Verifica que una agencia existe y está activa en dbintranet */
    private function _existeAgencia(int $id): bool
    {
        $stmt = $this->connect->prepare(
            "SELECT idAgencia FROM dbintranet.agencia WHERE idAgencia = ?"
        );
        $stmt->execute([$id]);
        return (bool)$stmt->fetchColumn();
    }

    /** Verifica que un departamento existe en dbintranet */
    private function _existeDepartamento(int $id): bool
    {
        $stmt = $this->connect->prepare(
            "SELECT idDepartamentoCooperativa
         FROM dbintranet.departamentocooperativa
         WHERE idDepartamentoCooperativa = ?"
        );
        $stmt->execute([$id]);
    ...
    /**
     * Verifica si una agencia ya tiene una bodega activa.
     * $excluirId = 0 en creación; id de la bodega actual en edición.
     */
    private function _agenciaTieneBodegaActiva(int $idAgencia, int $excluirId): bool
    {
        $stmt = $this->connect->prepare(
            "SELECT id FROM bodega_inventario.bodegas
         WHERE id_tipo = 1 AND id_agencia = ? AND activo = 1 AND id != ?"
        );
    ...
    /**
     * Verifica si un departamento ya tiene una bodega activa.
     */
    private function _departamentoTieneBodegaActiva(int $idDepto, int $excluirId): bool
    {
        $stmt = $this->connect->prepare(
            "SELECT id FROM bodega_inventario.bodegas
         WHERE id_tipo = 2 AND id_departamento_cooperativa = ? AND activo = 1 AND id != ?"
        );
...
// ============================================================================
//? ENCARGADOS DE BODEGA DE ÁREA — Funciones para agregar a la clase existente
// ============================================================================
// Tabla: bodega_inventario.inv_encargados_bodega_area
//   - PK: id_usuario (CHAR 30) → un usuario solo puede ser encargado de una bodega
//   - Una bodega puede tener varios encargados
//   - Solo aplica a bodegas de área (id_tipo = 2)
//   - No eliminar, solo activar/desactivar
//
// NOTA: Se asume que la tabla de datos personales es dbintranet.datospersonales
//       Ajustar si el nombre es diferente (ej. datos_personales, datosPersonales)
// ============================================================================


// ── GET: listarEncargados ─────────────────────────────────────────────────────

    /**
     * Lista todos los encargados con datos del usuario y bodega asignada.
     *
     * GET bodega/encargados/listarEncargados
     * Params opcionales: ?id_bodega=X  &activo=0|1
     */
    public function listarEncargados(): array
    {
        try {
            ...
            if ($idBodega !== null && $idBodega !== false) {
            ...
            return $this->res->ok('Encargados obtenidos', [
                'encargados' => $encargados,
                'total' => count($encargados),
            ]);
        } catch (Exception $e) {
            error_log("[Encargados] listarEncargados: " . $e->getMessage());
            return $this->res->fail('Error al obtener los encargados', $e);
        }
    }


// ── GET: buscarUsuarios?busqueda=X ────────────────────────────────────────────

    /**
     * Búsqueda live de usuarios para el selector del modal de asignación.
     * Mínimo 2 caracteres. Devuelve hasta 20 resultados.
     * Filtra usuarios activos (idEstados = 1).
     *
     * GET bodega/encargados/buscarUsuarios?busqueda=juan
     */
    public function buscarUsuarios(): array
    {
        try {
            ...
            if (mb_strlen($busqueda) < 2) {
                return $this->res->fail('Ingrese al menos 2 caracteres para buscar');
            }
            ...
            return $this->res->ok('Usuarios encontrados', [
                'usuarios' => $usuarios,
                'total' => count($usuarios),
            ]);
        } catch (Exception $e) {
            error_log("[Encargados] buscarUsuarios: " . $e->getMessage());
            return $this->res->fail('Error al buscar usuarios', $e);
        }
    }


// ── POST: asignarEncargado ────────────────────────────────────────────────────

    /**
     * Asigna un usuario como encargado de una bodega de área.
     *
     * Comportamiento:
     *   - Si el usuario no tiene registro → INSERT
     *   - Si ya tiene registro (misma bodega) → info() "ya es encargado"
     *   - Si ya tiene registro (diferente bodega) → UPDATE (reasignación)
     *
     * POST bodega/encargados/asignarEncargado
     * Body: { "id_usuario": "USR001", "id_bodega": 3 }
     *
     * @param object|array $datos
     */
    public function asignarEncargado($datos): array
    {
        try {
            ...
            $stmtUser = $this->connect->prepare(
                "SELECT u.idUsuarios, dp.nombres
             FROM dbintranet.usuarios u
             INNER JOIN dbintranet.datospersonales dp ON dp.idDatosPersonales = u.idDatosPersonales
            ...
            if (!$usuario) {
                return $this->res->fail('El usuario no existe o está inactivo');
            }

            // ── Verificar que la bodega existe, está activa y es de área ──────────
            $stmtBodega = $this->connect->prepare(
                "SELECT id, id_tipo, nombre FROM bodega_inventario.bodegas WHERE id = ? AND activo = 1"
            );
            ...
            if ((int)$bodega->id_tipo !== 2) {
                return $this->res->fail('Solo se pueden asignar encargados a bodegas de área');
            }

            // ── Verificar si ya existe un registro para este usuario ──────────────
            $stmtExiste = $this->connect->prepare(
                "SELECT id_usuario, id_bodega, activo
            ...
            if ($existente) {
                if ((int)$existente->id_bodega === $idBodega && (int)$existente->activo === 1) {
                    return $this->res->info(
                        "{$usuario->nombres} ya es encargado activo de la bodega \"{$bodega->nombre}\""
                    );
                }

                // Reasignar o reactivar
                $this->connect->prepare(
                    "UPDATE bodega_inventario.inv_encargados_bodega_area
                 SET id_bodega = ?, activo = 1
                 WHERE id_usuario = ?"
                )->execute([$idBodega, $idUsuario]);
            ...
            $this->connect->prepare(
                "INSERT INTO bodega_inventario.inv_encargados_bodega_area (id_usuario, id_bodega)
             VALUES (?, ?)"
            )->execute([$idUsuario, $idBodega]);

            return $this->res->ok("Encargado asignado correctamente a bodega \"{$bodega->nombre}\"");
        } catch (Exception $e) {
            error_log("[Encargados] asignarEncargado: " . $e->getMessage());
            return $this->res->fail('Error al asignar encargado', $e);
        }
    }


// ── POST: toggleEncargado ─────────────────────────────────────────────────────

    /**
     * Activa o desactiva un encargado.
     * Al desactivar, el usuario pierde acceso a la gestión de la bodega.
     *
     * POST bodega/encargados/toggleEncargado
     * Body: { "id_usuario": "USR001", "activo": 0 }
     *
     * @param object|array $datos
     */
    public function toggleEncargado($datos): array
    {
        try {
            ...
            if (!$encargado) {
                return $this->res->fail('El encargado no existe');
            }
            if ((int)$encargado->activo === $activo) {
                $estado = $activo === 1 ? 'activo' : 'inactivo';
                return $this->res->info("El encargado ya se encuentra {$estado}");
            }

            $this->connect->prepare(
                "UPDATE bodega_inventario.inv_encargados_bodega_area SET activo = ? WHERE id_usuario = ?"
            )->execute([$activo, $idUsuario]);

            $mensaje = $activo === 1 ? 'Encargado activado correctamente' : 'Encargado desactivado correctamente';
            return $this->res->ok($mensaje);
        } catch (Exception $e) {
            error_log("[Encargados] toggleEncargado: " . $e->getMessage());
...
// ── POST: agregarProductoCelda ────────────────────────────────────────────────

    /**
     * POST bodega/miMatriz/agregarProductoCelda
     * Body: { "id_matriz": 5, "id_producto": 12 }
     *
     * @param object|array $datos
     */
    public function agregarProductoCelda($datos): array
    {
        try {
            ...
            if ($idMatriz < 1)   return $this->res->fail('id_matriz es requerido');
            ...
            $stmt = $this->connect->prepare(
                "INSERT IGNORE INTO bodega_inventario.matriz_acceso_productos
                 (id_matriz, id_producto, por_categoria) VALUES (?, ?, 0)"
            );
            $stmt->execute([$idMatriz, $idProducto]);

            if ($stmt->rowCount() === 0) {
                return $this->res->info('El producto ya estaba en la celda');
            }
            return $this->res->ok('Producto agregado');
        } catch (Exception $e) {
            error_log("[MiMatriz] agregarProductoCelda: " . $e->getMessage());
            return $this->res->fail('Error al agregar el producto', $e);
        }
    }


// ── POST: agregarCategoriaCelda ───────────────────────────────────────────────

    /**
     * Agrega todos los productos activos de una categoría a la celda.
     *
     * POST bodega/miMatriz/agregarCategoriaCelda
     * Body: { "id_matriz": 5, "id_categoria": 3 }
     *
     * @param object|array $datos
     */
    public function agregarCategoriaCelda($datos): array
    {
        try {
            ...
            if ($idMatriz < 1)    return $this->res->fail('id_matriz es requerido');
            if ($idCategoria < 1) return $this->res->fail('id_categoria es requerido');
            ...
            if ($insertados === 0) {
                return $this->res->info(
                    'Todos los productos de la categoría ya estaban en la celda'
                );
            }
            return $this->res->ok("Se agregaron {$insertados} producto(s)");
        } catch (Exception $e) {
            error_log("[MiMatriz] agregarCategoriaCelda: " . $e->getMessage());
            return $this->res->fail('Error al agregar la categoría', $e);
        }
    }


// ── POST: eliminarProductoCelda ───────────────────────────────────────────────

    /**
     * POST bodega/miMatriz/eliminarProductoCelda
     * Body: { "id": 12 }  ← id de matriz_acceso_productos
     *
     * @param object|array $datos
     */
    public function eliminarProductoCelda($datos): array
    {
        try {
            $datos = $this->limpiarDatos($datos);
            $id    = (int) ($datos->id ?? 0);
            if ($id < 1) return $this->res->fail('id es requerido');

            $idBodega = $this->_obtenerBodegaDelEncargado($this->idUsuario);
            if (!$idBodega) return $this->res->fail('No eres encargado activo de ninguna bodega');

            // Verificar que el producto pertenece a una celda de mi bodega
            $stmt = $this->connect->prepare(
                "SELECT map.id
                 FROM bodega_inventario.matriz_acceso_productos map
                 INNER JOIN bodega_inventario.matriz_acceso ma ON ma.id = map.id_matriz
                 WHERE map.id = ? AND ma.id_bodega = ?"
            );
            $stmt->execute([$id, $idBodega]);
            if (!$stmt->fetchColumn()) {
                return $this->res->fail('Este producto no pertenece a tu bodega');
            }

            $this->connect->prepare(
                "DELETE FROM bodega_inventario.matriz_acceso_productos WHERE id = ?"
            )->execute([$id]);

            return $this->res->ok('Producto eliminado de la celda');
        } catch (Exception $e) {
            error_log("[MiMatriz] eliminarProductoCelda: " . $e->getMessage());
            return $this->res->fail('Error al eliminar el producto', $e);
        }
    }
...
// ── Helper privado ────────────────────────────────────────────────────────────
    ...
    /**
     * Verifica que una celda (id de matriz_acceso) pertenezca a la bodega dada.
     * Critical para prevenir manipulación de IDs ajenos.
     */
    private function _celdaPertenece(int $idMatriz, int $idBodega): bool
    {
        $stmt = $this->connect->prepare(
            "SELECT id FROM bodega_inventario.matriz_acceso
    ...
    // ============================================================================
//? NUEVO ENDPOINT: asignarProductoACeldas
// ============================================================================
// Agrega un producto a múltiples celdas activas de la matriz en una sola
// transacción. Resuelve el flujo inverso: "tengo este producto, quiero que
// esté disponible en estas N combinaciones puesto+agencia".
//
// Agregar al array $metodosPost de la clase:
//   'asignarProductoACeldas'
//
// POST bodega/miMatriz/asignarProductoACeldas
// Body: {
//   "id_producto": 12,
//   "celdas": [5, 8, 13, 21]   ← IDs de matriz_acceso
// }
// ============================================================================

    /**
     * Asigna un producto a múltiples celdas activas de la bodega en una sola
     * transacción.
     *
     * Validaciones:
     *   - id_producto requerido y debe existir en productos.
     *   - celdas: array no vacío de enteros positivos (máx. 200).
     *   - Todas las celdas deben pertenecer a la bodega del encargado autenticado
     *     (verificación en lote con un solo IN para evitar N queries).
     *   - INSERT IGNORE para manejar duplicados silenciosamente.
     *
     * Respuesta success:
     *   { insertados: N, omitidos: M, total_enviados: N+M }
     *
     * Respuesta info (todos ya existían):
     *   { mensaje: "El producto ya estaba en todas las celdas seleccionadas" }
     *
     * @param object|array $datos
     */
    public function asignarProductoACeldas($datos): array
    {
        try {
            ...
            // El campo "celdas" llega como array JSON
            ...
            if ($idProducto < 1) {
                return $this->res->fail('id_producto es requerido');
            }

            if (!is_array($celdasRaw) || empty($celdasRaw)) {
                return $this->res->fail('Debes seleccionar al menos una celda');
            }

            // Sanitizar IDs de celdas
            $idsCeldas = array_values(
                array_unique(
                    array_filter(
                        array_map('intval', $celdasRaw),
                        fn($id) => $id > 0
                    )
                )
            );
            ...
            if (empty($idsCeldas)) {
                return $this->res->fail('No se encontraron IDs de celda válidos');
            }

            if (count($idsCeldas) > 200) {
                return $this->res->fail('El máximo por llamada es 200 celdas');
            }

            // ── Verificar que el producto existe ──────────────────────────────

            $stmtProd = $this->connect->prepare(
                "SELECT id FROM bodega_inventario.productos WHERE id = ? AND activo = 1"
            );
            $stmtProd->execute([$idProducto]);
            if (!$stmtProd->fetchColumn()) {
                return $this->res->fail('El producto no existe o está inactivo');
            }

            // ── Verificar que TODAS las celdas pertenecen a la bodega ─────────
            // Un solo query con IN es mucho más eficiente que _celdaPertenece() N veces.
            ...
            $placeholdersCeldas = implode(',', array_fill(0, count($idsCeldas), '?'));
            $paramsBodega       = array_merge([$idBodega], $idsCeldas);

            $stmtVerif = $this->connect->prepare(
                "SELECT COUNT(*) FROM bodega_inventario.matriz_acceso
             WHERE id_bodega = ? AND id IN ({$placeholdersCeldas}) AND activo = 1"
            );
            $stmtVerif->execute($paramsBodega);
            $encontradas = (int) $stmtVerif->fetchColumn();

            if ($encontradas !== count($idsCeldas)) {
                return $this->res->fail(
                    'Una o más celdas no pertenecen a tu bodega o están inactivas'
                );
            }

            // ── Inserción en lote ─────────────────────────────────────────────
            // Una fila por celda: (id_matriz, id_producto, por_categoria = 0)

            $this->connect->beginTransaction();

            $placeholdersInsert = implode(
                ', ',
                array_fill(0, count($idsCeldas), '(?, ?, 0)')
            );
            ...
            $insertados    = $stmt->rowCount();
            $totalEnviados = count($idsCeldas);
            $omitidos      = $totalEnviados - $insertados;
            ...
            if ($insertados === 0) {
                return $this->res->info(
                    'El producto ya estaba en todas las celdas seleccionadas',
                    null,
                    [
                        'insertados'     => 0,
                        'omitidos'       => $omitidos,
            ...
            $mensaje = $insertados === $totalEnviados
                ? "Producto asignado a {$insertados} celda(s) correctamente"
                : "Producto asignado a {$insertados} celda(s). {$omitidos} ya lo tenían y se omitieron";
            ...
            error_log("[MiMatriz] asignarProductoACeldas: " . $e->getMessage());
            return $this->res->fail('Error al asignar el producto', $e);
        }
    }
// ============================================================================
